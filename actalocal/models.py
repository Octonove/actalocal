"""Modelos Whisper (GGML de whisper.cpp). Reutiliza los de TranscriptorIA o
CapturaStudio si ya estan descargados (config.models_dir apunta alli cuando
existen), evitando descargas duplicadas."""
...
MODELS = {
    "tiny":   ("ggml-tiny.bin",    75, "Rapido · poca precision"),
    "base":   ("ggml-base.bin",   142, "Equilibrado"),
    "small":  ("ggml-small.bin",  466, "Buena precision (recomendado)"),
    "medium": ("ggml-medium.bin", 1536, "Maxima precision · mas lento"),
}
ORDER = ["tiny", "base", "small", "medium"]


def recommend_model(ram_gb: float) -> str:
    """Modelo Whisper sugerido segun la RAM del equipo (mas grande = mejor texto)."""
    if ram_gb >= 16:
        return "medium"
    if ram_gb >= 8:
        return "small"
...
def _download_curl(url: str, tmp: str) -> bool:
    ...
    try:
        r = subprocess.run([curl, "-fL", "--ssl-no-revoke", "--retry", "2",
                            "--connect-timeout", "30", "-A", "ActaLocal", "-o", tmp, url],
...
def _download_urllib(url: str, tmp: str, progress_cb=None) -> bool:
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "ActaLocal"})
