# Construye el ejecutable de ActaLocal con PyInstaller (onedir).
# Empaqueta el ffmpeg 'full' de Gyan (trae el filtro whisper) para ser portable.
...
Write-Host "== Localizando FFmpeg (build full con whisper) ==" -ForegroundColor Cyan
...
Push-Location $root
& $py -m PyInstaller --noconfirm --clean (Join-Path $PSScriptRoot "ActaLocal.spec")
$code = $LASTEXITCODE
Pop-Location

if ($code -eq 0) {
    Write-Host "`n== LISTO ==" -ForegroundColor Green
    Write-Host "Ejecutable: $(Join-Path $root 'dist\ActaLocal\ActaLocal.exe')"
} else {
